...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const CONFIG = {
  // أرقام الحسابات والمحافظ (هذه أرقام تجريبية — بدلها بالأرقام الحقيقية!)
  bankAccounts: [
    { brand: "مصرف الرافدين", type: "حوالة مصرفية", number: "1234567890",    holder: "روان" },
    { brand: "مصرف الرشيد",  type: "حوالة مصرفية", number: "1230007890123", holder: "حسين" },
  ],
  ewallets: [
    { brand: "زين كاش",   type: "محفظة إلكترونية", number: "0780-xxx-xxxx", holder: "روان" },
    { brand: "كي كارد (Qi)", type: "محفظة إلكترونية", number: "0770-xxx-xxxx", holder: "حسين" },
  ],
  // عنوان استلام الهدايا العينية
  giftAddress: {
    receiver: "روان",
    phone: "0780-xxx-xxxx",
    address: "بغداد، المنصور، شارع 14 رمضان، رقم الدار 10",
  },
  ...
  // الموسيقى الخلفية (يوتيوب)
  youtubeVideoId: "lHlxpGnAGbU",

  // تاريخ ووقت عقد القران (توقيت بغداد +03:00) — يستخدمه العد التنازلي والتقويم
  weddingDateISO: "2026-10-02T08:00:00+03:00",
};
...
function initGuestName() {
  ...
  const el = $("#guest-name");
  if (el) el.textContent = name || "ضيفنا العزيز";
  const input = $("#f-nama");
...
function initMusic() {
      ...
      events: {
        ...
        onStateChange: (e) => {
          ...
          if (e.data === YT.PlayerState.PLAYING) {
            ...
            btn?.setAttribute("aria-label", "إيقاف الموسيقى مؤقتاً");
  ...
  btn?.addEventListener("click", () => {
    if (!playerReady) {
      wantMusic = true;
      toast("الموسيقى دتحمّل، جرب بعد شوية…");
      return;
    }
...
function initGallery() {
  ...
  items.forEach((el, i) => {
    ...
    el.parentElement.setAttribute("aria-label", "كبّر الصورة: " + (el.alt || "المعرض"));
...
function giftCardHTML({ brand, type, number, holder }, icon) {
  return `
    <article class="gift-card card">
      ...
      <p class="gift-holder">باسم: ${escapeHTML(holder)}</p>
...
function initGift() {
  ...
  if (addrWrap) {
    ...
    addrWrap.innerHTML = `
      <article class="gift-card card gift-card--address">
        <div class="gift-head">
          <span class="gift-brand"><span class="g-dot"></span>هدية عينية</span>
          <span class="gift-type">عنوان التوصيل</span>
        </div>
        <p class="gift-number">${escapeHTML(a.address)}</p>
        <p class="gift-holder">المستلم: ${escapeHTML(a.receiver)} &middot; ${escapeHTML(a.phone)}</p>
  ...
  document.addEventListener("click", async (e) => {
    ...
    toast(ok ? "تم النسخ ✓" : "ما انضبط النسخ، انسخه يدوي رجاءً.");
  });
}
...
const SEED_MESSAGES = [
  { name: "أهل العروس", attend: "أحضر", guests: "2", message: "ألف مبروك حسين وروان! الله يتمم لكم على خير ويجعل أيامكم كلها فرح.", ts: Date.now() - 3 * 86400000 },
  { name: "صديق حسين", attend: "أحضر", guests: "1", message: "أخيراً يا معوّد! بارك الله لكما وبارك عليكما وجمع بينكما في خير. نشوفكم بيوم الفرحة!", ts: Date.now() - 2 * 86400000 },
  { name: "زملاء الدراسة", attend: "مو متأكد", guests: "1", message: "إن شاء الله نلحگ الفرحة! كل التوفيق والمحبة للعرسان.", ts: Date.now() - 86400000 },
];
...
function badgeClass(attend) {
  if (attend === "أحضر") return "b-hadir";
  if (attend === "معتذر") return "b-berhalangan";
  return "b-ragu";
}

function formatDate(ts) {
  try {
    return new Date(ts).toLocaleDateString("ar-IQ", { day: "numeric", month: "short", year: "numeric" });
  } catch { return ""; }
}
...
function renderMessages() {
  ...
  all.forEach((m) => {
    ...
    name.textContent = m.name || "بدون اسم";
    ...
    time.textContent = formatDate(m.ts) + (m.guests && m.guests !== "0" ? ` · عدد الضيوف: ${m.guests}` : "");
...
function initRSVP() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!nama) { toast("رجاءً اكتب اسمك أول."); form.nama.focus(); return; }
    if (!pesan) { toast("اكتب تهنئتك ودعوتك للعرسان 🤍"); form.pesan.focus(); return; }
